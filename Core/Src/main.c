...
float Kp = 0.1f;             // 比例增益
float Ki = 2.0f;            // 积分增益
float target_voltage = 8.5f; // 目标电压
float error, integral, control_output;

void PI_Controller()
{
  // 计算误差：目标电压 - 实际电压
  error = target_voltage - real_voltage;
  ...
  // 更新PWM占空比
  __HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_1, (uint32_t)control_output);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    adc_value = HAL_ADC_GetValue(&hadc1);             // 读取ADC转换结果
    real_voltage = adc_value * 3.3f / 4096 * 1050 * 20000 / 2500 / 1000;
    PI_Controller();
    /* USER CODE END WHILE */
